export interface StatCell {
  ...
  sub?: string;
  highlight?: boolean; // deprecated — no longer changes styling; cards render uniformly
}
    ...
    <div className="flex divide-x divide-border rounded-[10px] border border-border">
      {stats.map((s, i) => (
        <div key={i} className="min-w-0 flex-1 px-4 py-3">
          ...
          {s.delta && (
            <div className={`mt-0.5 text-[11.5px] font-medium ${s.delta.good ? "text-positive" : "text-negative"}`}>{s.delta.text}</div>
          )}
